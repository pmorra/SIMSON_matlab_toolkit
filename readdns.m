...
%
% ***********************************************************************
function [vel,xF,yF,zF,Lx,Ly,Lz,t,Re,flowtype,dstar,pou,rlam,spanv]=readdns(filename)
%
...
disp(' - Reading u');
rlu = zeros(NNx,NNz,NNy);
ilu = zeros(NNx,NNz,NNy);
for indz=1:NNz
  for indy=1:NNy
    fread(fid,1,'int');
...
disp(' - Reading v');
rlv = zeros(NNx,NNz,NNy);
ilv = zeros(NNx,NNz,NNy);
for indz=1:NNz
  for indy=1:NNy
    fread(fid,1,'int');
    vec=fread(fid,NNx*2,'float64');
...
disp(' - Reading w');
rlw = zeros(NNx,NNz,NNy);
ilw = zeros(NNx,NNz,NNy);
for indz=1:NNz
  for indy=1:NNy
    fread(fid,1,'int');
    vec=fread(fid,NNx*2,'float64');
...
NxF=2*NNx;
NzF=NNz;
xF=Lx/NxF*(-floor(NxF/2):1:floor(NxF/2))';
if NzF > 1
